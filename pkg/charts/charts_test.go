...
var _ = Describe("Chart package test", func() {
	...
	DescribeTable("#ComputeCalicoChartValues",
		func(config func() *calicov1alpha1.NetworkConfig, configResult func() *calicov1alpha1.NetworkConfig, wantsVPA bool,
			...
			values, err := chartspkg.ComputeCalicoChartValues(network, config(), kubernetesVersion, wantsVPA, kubeProxyEnabled, false, nodesFunc(), []string{network.Spec.PodCIDR}, false)
			Expect(err).To(BeNil())
	...
	Describe("#ComputeCalicoChartValues", func() {
		...
		DescribeTable("should correctly compute calico chart values with non-privileged mode enabled",
			func(config func() *calicov1alpha1.NetworkConfig, expectedResult bool) {
				values, err := chartspkg.ComputeCalicoChartValues(network, config(), kubernetesVersion, true, true, true, &nodeCIDR, nil, false)
				Expect(err).To(BeNil())
		...
		It("should error on invalid config value", func() {
			_, err := chartspkg.ComputeCalicoChartValues(network, networkConfigInvalid, kubernetesVersion, true, true, false, &nodeCIDR, nil, false)
		...
		Context("IPv4", func() {
			...
			It("should correctly configure for IPv4 networks", func() {
				values, err := chartspkg.ComputeCalicoChartValues(
					network,
					nil, "", false, false, false, nil, nil, false,
				)
		...
		Context("IPv6", func() {
			...
			It("should correctly configure for IPv6 networks", func() {
				values, err := chartspkg.ComputeCalicoChartValues(
					network,
					nil, "", false, false, false, nil, []string{"2001:0db8:85a3:0000::/56"}, true,
				)
				Expect(err).NotTo(HaveOccurred())
			...
			It("should use overrides from the config", func() {
				...
				values, err := chartspkg.ComputeCalicoChartValues(
					network, config,
					"", false, false, false, nil, []string{"2001:0db8:85a3:0000::/56"}, true,
				)
				Expect(err).NotTo(HaveOccurred())
		...
		Context("Dual-stack", func() {
			...
			It("should correctly configure for Dual-stack networks", func() {
				values, err := chartspkg.ComputeCalicoChartValues(
					network,
					nil, "", false, false, false, nil, []string{"2001:0db8:85a3:0000::/56", podCIDR}, true,
				)
				Expect(err).NotTo(HaveOccurred())
	...
	Describe("#RenderCalicoChart", func() {
		...
		DescribeTable("Render Calico charts correctly",
			func(nodes *string) {
				...
				_, err := chartspkg.RenderCalicoChart(mockChartRenderer, network, networkConfigNil, kubernetesVersion, false, true, false, nodes, nil, false)
				Expect(err).NotTo(HaveOccurred())
